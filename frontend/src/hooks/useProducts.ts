...
export const useProducts = () => {
    ...
    const getProduct = async (id: number) => {
        const response = await fetch(
            API_ENDPOINTS.product.getDetails.replace('{id}', id.toString()), {
                credentials: 'include'
            }
        );
        return response.json();
    ...
    const updateProduct = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: ProductUpdateData }) => {
            ...
            const response = await fetch(
                API_ENDPOINTS.product.update.replace('{id}', id.toString()), {
                    method: 'PUT',
                    credentials: 'include',
                    body: formData
                }
    ...
    const deleteProduct = useMutation({
        mutationFn: async (id: number) => {
            const response = await fetch(
                API_ENDPOINTS.product.delete.replace('{id}', id.toString()), {
                    method: 'DELETE',
                    credentials: 'include'
                }
            );
